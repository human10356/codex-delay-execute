...
Final audit on 2026-09-15 rechecked the durable task JSON (`status=attempted`, `runtime_status=completed`, `attempt_count=1`, `execution_mode=non_git`), the five unique history stages, one service start in the journal, a disabled/inactive timer, an inactive successful service with zero restarts, and a free task lock. This confirms one catch-up delivery after a full `wsl --shutdown`, not merely a distribution userspace restart. The annotated beta.3 release tag has since been pushed; anonymous public installation remains a separate publication gate.

On 2026-09-15, a separate Ubuntu 24.04.5 WSL distribution with Codex CLI 0.154.0, Python 3.12.3, Git 2.43.0, and a running user systemd manager tested the beta.3 tag without GitHub credentials. A local archive of the pushed tag was copied into that distribution and verified by SHA-256 before extraction. The local Marketplace and plugin installed as `0.1.0-beta.3+codex.20260914181621`; Python compilation and all 34 tests passed. In a non-Git directory, a fresh Codex runtime discovered the enabled skill and trusted bundled `UserPromptSubmit` hook. An ephemeral CLI invocation executed `$delay-execute list` and returned no tasks. The first read-only-sandbox attempt could not initialize the writable plugin-data directory; a second invocation with workspace-write access limited to that directory succeeded. No timer or detached writer remained. Credential-disabled HTTPS access to the still-private repository was rejected, as expected; this does not satisfy the later anonymous public-install gate.

After a new, repository-scoped, read-only SSH deploy key was registered on GitHub for that distribution, SSH authentication and private tag reading succeeded. Port 22 did not return a host key during the initial check, so the test used GitHub's documented SSH endpoint on port 443 after matching its host-key fingerprint to GitHub's published value. The tag peeled to commit `3cbaf59dd9d9789be687bcf60dcc2d5ebd77d9dc`. In an isolated temporary Codex home, `codex plugin marketplace add` installed the SSH Git source pinned to `v0.1.0-beta.3`, and `codex plugin add` installed the plugin. The Marketplace checkout HEAD matched the peeled tag commit; release validation, Python compilation, and all 34 tests passed from that checkout. App-server discovery reported the installed skill and enabled hook with no discovery errors. The hook was `untrusted` in this fresh home and was not executed there. Codex warned that temporary Codex homes cannot create PATH aliases and that system bubblewrap was absent, so it used its bundled copy; neither warning was suppressed. No task timer or test app-server process was left running. GitHub write denial was not tested; read-only permission depends on the Deploy keys setting remaining unchecked for write access.
