...
适用场景包括：第二天早上生成项目摘要、构建完成后复查结果、稍后重新检查失败测试，以及每日或每周的维护提示。插件只负责恢复创建任务的那条 Codex 对话，不是通用的主机级任务调度器。
...
- 同一个 WSL 发行版重新启动后，只要 Linux 文件系统、Codex home 和用户级 systemd 状态仍在，任务可以补执行。
- 发行版停止期间不会运行任务；任务也不会自动迁移到另一套 WSL 发行版、Windows、macOS 或另一 Linux 用户。
- 如需在另一环境使用，请在该环境重新安装插件，并从该环境可访问的对话重新创建任务；原任务仍归属于创建它的环境。
...
卸载前先执行 `$delay-execute list`，对每个任务依次执行 `cancel` 和精确的 `confirm-cancel`。取消完成后再移除插件：

```bash
codex plugin remove delay-execute@<marketplace-name>
```

如果不再使用该 Marketplace，还可以移除来源：

```bash
codex plugin marketplace remove <marketplace-name>
```

插件卸载只移除插件缓存，不会自动清理已经创建的用户级 systemd unit、runner 快照或保留的任务数据。如果插件已无法加载，请根据任务记录中的确切 unit 名称执行：

```bash
systemctl --user disable --now <name>.timer <name>.service
```

然后用 `systemctl --user list-timers` 和 `ps` 确认没有残留任务或 Codex 进程；不要使用宽泛的通配符批量删除。
